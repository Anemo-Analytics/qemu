...
| **7** | Filesystem available | **`/fs/` mounted, `/fs/etc/startup.app` loadable** | BSP doesn't print "Cannot find startup.app in runmode !" | 🟡 **Phase 4a scaffolding installed 2026-05-01.** Era pinned (2022, see `SESSION_LOG_2026-05-01.md`); file-IO entry NIPs identified (existence check `0x13ffe4`, fopen `0x2a5508`, open `0x2b13c8`, script-runner `0x1061c0`); 8-byte stub installed at `0x13ffe4` making the existence helper always return 1. Verification inconclusive — no BSP serial capture in current `run.sh`, and the 100 µs diag sampler misses the 2-cycle stub, so we can't yet *see* whether the bypass propagates. Vestas-mode flag still doesn't flip → modules aren't loading regardless. Phase 4b (real open/read/close proxy) deferred — likely an MMIO-hypercall region in unused MBAR space. |
