...
| **7** | Filesystem available | **`/fs/` mounted, `/fs/etc/startup.app` loadable** | BSP doesn't print "Cannot find startup.app in runmode !" | 🟡 **Phase 4b infrastructure complete 2026-05-02 — but unreached.** MMIO-hypercall doorbell at `0xF0004000`, host-side `open`/`read`/`close`/`lseek`/`fstat` handlers, fd-proxy table, 9-instruction stubs at the BSP's `open=0x2b13c8` / `read=0x2b18fc` / `close=0x2b17d4` wrappers. (Plan's `0x2b20a8` for read was wrong — that's a 2-slot global setter; real `read` found via the embedded VxWorks symbol table at file `0x7f60xx`.) Host root: `MPC5200_FS_ROOT` env, default `/home/kasper/Vestas/.../node10_roye2/fs/`. **Empirical finding (75 s of guest, full diag-sampler window):** stations `0x13ffe4` / `0x14c7d4` / `0x1061c0` / `0x2b13c8` / `0x2a5508` all UNREACHED. Zero `FS_HOOK:` lines fire. The whole runmode init path (`ctvxInit` 0x14cfac → `ctvxInitTurbine` 0x14bc14 → existence check → script runner) is never executed. **Gate 7 is now known to be gated by an upstream selector** (likely runmode storage in EEPROM / bootline / external probe), not the FS layer. Phase-4b code is correct and ready; we just need to trip the upstream gate. See `SESSION_LOG_2026-05-02.md`. |
