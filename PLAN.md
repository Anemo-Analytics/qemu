...
Rough progress to gate 12: **~40-45% (2026-05-06).** Gate 7 closed
...
stays PEND'd. **2026-05-05:** Phase 0 diagnostics confirmed the BSP
doesn't use a mask register to gate SDMA (MainMask 0x510 is 0 = all
enabled); Phase 1.B (edge re-trigger via 1→0→1 EXT pulse) did not
crack dispatch. MSR.EE=0 hold confirmed as the dispatch wall.
**2026-05-06:** Sidestepped EXT dispatch entirely — installed a
BSP-side semGive shim (sysClkInt tail-patch + FSHOOK doorbell). The
shim works mechanically end-to-end (synthetic doorbell at vt=8s →
shim reads/clears it, tail-calls `semGive(0x07bee080)` at
`0x002ff5c4`) but **`tFecEndRx` still stays PEND**. New visible
