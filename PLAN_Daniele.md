...
- **0x207fe8 is not a park address.** It is the `isync` on the fast-exit
  ...
  by the tick handler) — the CPU is not stalled there.
- **Real idle condition:** the null/idle task runs the `windExit` spin
...
- **FTP server is 169.254.254.252**, not .253 — .253 does not appear in
  the binary.
...
- **Secondary FS question (open):** how `/ata0a/` is mounted was not
  fully resolved; see Open Questions in `BSP_park_findings.md`.
